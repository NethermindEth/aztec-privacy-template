...
async function buildUniswapTestContext(): Promise<UniswapTestContext> {
	...
	const aztecState = await provisionPrivateTokenBalance(
		"Uniswap Privacy Token",
		"UPT",
		5_000n,
	);
	const content = castKeccak(
		`uniswap-swap:${aztecState.ownerAddress}:${aztecState.balance}`,
	);
	...
	castSend(USER_PRIVATE_KEY, router, "setSimulatedAmountOut(uint256)", [
...
const UNISWAP_FLOW: ProtocolFlowSpec = {
	...
	buildContent: (ownerAddress, balance) =>
		`uniswap-swap:${ownerAddress}:${balance}`,
	portal: {
		source:
			"packages/protocols/uniswap/solidity/UniswapPortal.sol:UniswapPortal",
	...
	setup: ({ mocks }) => {
		castSend(
			USER_PRIVATE_KEY,
			mocks.UNISWAP_MOCK_ROUTER,
			"setSimulatedAmountOut(uint256)",
			[SIMULATED_AMOUNT_OUT],
		);
	},
	request: {
	...
	assertState: ({ mocks, userAddress }) => {
		const lastAmountInRaw = castCall(
			mocks.UNISWAP_MOCK_ROUTER,
			"lastAmountIn()(uint256)",
		);
		...
		const lastRecipient = castCall(
			mocks.UNISWAP_MOCK_ROUTER,
			"lastRecipient()(address)",
		);
...
test(
	"Uniswap E2E: unauthorized relayer cannot execute swap",
	{ timeout: 900_000 },
	async () => {
...
test(
	"Uniswap E2E: request validation rejects zero recipient",
	{ timeout: 900_000 },
	async () => {
		await withUniswapFlowTeardown((context) => {
			assert.throws(() => {
...
test(
	"Uniswap E2E: request mismatch blocks execution",
	{ timeout: 900_000 },
	async () => {
		await withUniswapFlowTeardown((context) => {
			castSend(
...
test(
	...
	async () => {
		await withUniswapFlowTeardown((context) => {
			castSend(
				USER_PRIVATE_KEY,
				context.mocks.UNISWAP_MOCK_ROUTER,
				"setShouldFail(bool)",
				["true"],
			);
			castSend(
				USER_PRIVATE_KEY,
				context.mocks.UNISWAP_MOCK_TOKEN_IN,
				"mint(address,uint256)",
				[context.portalAddress, "1000"],
			);
