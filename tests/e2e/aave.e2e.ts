...
function aaveWithdrawExecuteArgs(content: string, amount = AAVE_REQUEST_AMOUNT): string[] {
...
test(
  ...
  async () => {
    await withAaveFlowTeardown((context) => {
      castSend(USER_PRIVATE_KEY, context.mocks.AAVE_MOCK_POOL, 'setShouldFail(bool)', ['true']);
      castSend(USER_PRIVATE_KEY, context.mocks.AAVE_MOCK_ERC20, 'mint(address,uint256)', [
        context.portalAddress,
        AAVE_REQUEST_AMOUNT,
      ]);
      ...
      const escapeRequest = getEscapeRequest(context.portalAddress, requestHash);
      assert.equal(escapeRequest.depositor.toLowerCase(), USER_ADDRESS.toLowerCase());
      assert.equal(escapeRequest.token.toLowerCase(), context.mocks.AAVE_MOCK_ERC20.toLowerCase());
      ...
      assert.throws(() => {
        castSend(USER_PRIVATE_KEY, context.portalAddress, 'claimEscape(bytes32)', [requestHash]);
      });
      ...
      const claimedRequest = getEscapeRequest(context.portalAddress, requestHash);
