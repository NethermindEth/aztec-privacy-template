...
async function buildLidoTestContext(): Promise<LidoTestContext> {
	...
	const aztecState = await provisionPrivateTokenBalance(
		"Lido Privacy Receipt",
		"LPR",
		1_000n,
	);
	const content = castKeccak(
		`lido-stake:${aztecState.ownerAddress}:${aztecState.balance}`,
	);
...
const LIDO_FLOW: ProtocolFlowSpec = {
	...
	buildContent: (ownerAddress, balance) =>
		`lido-stake:${ownerAddress}:${balance}`,
	portal: {
...
test(
	"Lido E2E: Aztec private state + L1 Lido portal flow",
	{ timeout: 900_000 },
	async () => {
...
test(
	"Lido E2E: unauthorized relayer cannot execute stake",
	{ timeout: 900_000 },
	async () => {
...
test(
	"Lido E2E: request validation rejects zero recipient",
	{ timeout: 900_000 },
	async () => {
		await withLidoFlowTeardown((context) => {
			assert.throws(() => {
...
test(
	"Lido E2E: duplicate execute for same request is rejected",
	{ timeout: 900_000 },
	async () => {
		await withLidoFlowTeardown((context) => {
			castSend(
...
test(
	"Lido E2E: request mismatch blocks execution",
	{ timeout: 900_000 },
	async () => {
		await withLidoFlowTeardown((context) => {
			castSend(
