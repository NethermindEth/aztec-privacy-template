# Aave example (Aztec privacy adapter + L1 portal)

This folder is a minimal end-to-end pattern for adding Aztec privacy to an Aave-like protocol integration.
...
1. A user creates a private intent on Aztec (deposit or withdraw).
2. The Aztec contract emits a message to an L1 portal.
3. The L1 portal executes the protocol action (Aave supply/withdraw).
...
You can copy this structure and swap protocol-specific logic while keeping the privacy flow model.

## Files in this protocol

- `packages/protocols/aave/aztec/src/main.nr`
  - Aztec contract (`AavePrivacyAdapter`)
  - Builds private intent IDs and message content
  - Tracks pending deposit/withdraw intent state
  - Consumes L1->L2 completion messages to finalize intents

- `packages/protocols/aave/solidity/AavePortal.sol`
  - L1 portal contract
  - Accepts user requests (`requestDeposit`, `requestWithdraw`)
  - Enforces relayer-only execution (`executeDeposit`, `executeWithdraw`)
  - Calls Aave-like pool interface
  - Handles failure via `EscapeHatch` registration
...
- `tests/mocks/aave/solidity/MockAavePool.sol`
- `tests/mocks/aave/solidity/MockERC20.sol`
...
`make test-e2e` runs the real Aave flow in `tests/aave.ts` using local Aztec + local L1.
...
3. Keep strict request metadata binding (action, actor, amount, nonce).
