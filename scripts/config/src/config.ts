...
const ALLOWED_KEYS = ['template_version', 'metadata', 'privacy', 'runtime', 'addresses', 'modules'];
...
function ensureKnownKeys(parsed: unknown, source: string): asserts parsed is ConfigSections {
  ...
  ensureKnownSectionKeys(
    (parsed as ConfigSections).modules,
    ALLOWED_MODULE_KEYS,
    source,
    'modules',
  );
...
function mergeSections(
  ...
  const merged = {
    ...
    modules: {
      enableBorrow:
        override.modules?.enable_borrow ??
        base.modules?.enable_borrow ??
        DEFAULTS.modules.enable_borrow,
      enableRepay:
        override.modules?.enable_repay ??
        base.modules?.enable_repay ??
        DEFAULTS.modules.enable_repay,
      enableLp:
        override.modules?.enable_lp ?? base.modules?.enable_lp ?? DEFAULTS.modules.enable_lp,
      enableQueue:
        override.modules?.enable_queue ??
        base.modules?.enable_queue ??
        DEFAULTS.modules.enable_queue,
      enableYield:
        override.modules?.enable_yield ??
        base.modules?.enable_yield ??
        DEFAULTS.modules.enable_yield,
    },
  };
  ...
  const normalized: GeneratedConfig = {
    ...
    modules: {
      enableBorrow: requireBoolean(merged.modules.enableBorrow, 'modules.enable_borrow'),
      enableRepay: requireBoolean(merged.modules.enableRepay, 'modules.enable_repay'),
      enableLp: requireBoolean(merged.modules.enableLp, 'modules.enable_lp'),
      enableQueue: requireBoolean(merged.modules.enableQueue, 'modules.enable_queue'),
      enableYield: requireBoolean(merged.modules.enableYield, 'modules.enable_yield'),
    },
  };
...
export function privacyFlagsNoir(config: GeneratedConfig): string {
  ...
  const modules = [
    ['enable_borrow', config.modules.enableBorrow],
    ['enable_repay', config.modules.enableRepay],
    ['enable_lp', config.modules.enableLp],
    ['enable_queue', config.modules.enableQueue],
    ['enable_yield', config.modules.enableYield],
  ];

  const flags = [...privacy, ...modules]
...
export function protocolConstantsTs(config: GeneratedConfig): string {
  const lines = [
    ...
    '',
    `export const ENABLE_BORROW = ${config.modules.enableBorrow ? 'true' : 'false'};`,
    `export const ENABLE_REPAY = ${config.modules.enableRepay ? 'true' : 'false'};`,
    `export const ENABLE_LP = ${config.modules.enableLp ? 'true' : 'false'};`,
    `export const ENABLE_QUEUE = ${config.modules.enableQueue ? 'true' : 'false'};`,
    `export const ENABLE_YIELD = ${config.modules.enableYield ? 'true' : 'false'};`,
  ];
...
library PortalConstants {
  ...
  bool internal constant ENABLE_BORROW = ${config.modules.enableBorrow ? 'true' : 'false'};
  bool internal constant ENABLE_REPAY = ${config.modules.enableRepay ? 'true' : 'false'};
  bool internal constant ENABLE_LP = ${config.modules.enableLp ? 'true' : 'false'};
  bool internal constant ENABLE_QUEUE = ${config.modules.enableQueue ? 'true' : 'false'};
  bool internal constant ENABLE_YIELD = ${config.modules.enableYield ? 'true' : 'false'};
}\n`;
}
